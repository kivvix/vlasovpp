...
/* ------------------------------------------------------------------------- */
  field3d<double> f(boost::extents[16][16][32][16]);
  complex_field<double,3> hf(boost::extents[16][16][32][16]);
  ...
  for (std::size_t k_x=0 ; k_x<f.size(0) ; ++k_x ) {
    for (std::size_t k_y=0 ; k_y<f.size(1) ; ++k_y ) {
      for (std::size_t k_z=0 ; k_z<f.size(2) ; ++k_z ) {
  ...
  splitVP1dx3dv<double> Lie( f , f.range.len_z() );
  double current_t = 0.;
  times.push_back(0.);
  ...
  auto compute_electric_energy = [&]( const ublas::vector<double> & E ) {
    ...
    for ( const auto & ei : E ) { electric_energy += ei*ei*f.step.dz; }
      return std::sqrt(electric_energy);
  };
  ee.push_back(compute_electric_energy(E));
  ...
  c << monitoring::data( "ee_poisson3d.dat" , ee , dt_y );
