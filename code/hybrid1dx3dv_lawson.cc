...
/* ------------------------------------------------------------------------- */
      ...
      for ( auto k_y=0u ; k_y<c.Nvy ; ++k_y ) {
        ...
        for ( auto k_z=0u ; k_z<c.Nvz ; ++k_z ) {
          ...
          for ( auto i=0u ; i<c.Nz ; ++i ) {
            ...
            vxfdv[i] += v_x*fvxvyvz[i]*f.volumeV();
            vyfdv[i] += v_y*fvxvyvz[i]*f.volumeV();
    ...
    for ( auto k_x=0u ; k_x<c.Nvx ; ++k_x ) {
      ...
      for ( auto k_y=0u ; k_y<c.Nvy ; ++k_y ) {
        ...
        for ( auto k_z=0u ; k_z<c.Nvz ; ++k_z ) {
          ...
          fft::ifft( hf[k_x][k_y][k_z].begin() , hf[k_x][k_y][k_z].end() , fvxvyvz.begin() );
          for ( auto i=0u ; i<c.Nz ; ++i ) {
            ec_perp[i] += (v_x*v_x + v_y*v_y)*fvxvyvz[i]*f.volumeV();
  ...
  #define _hjfx(hf) v_x*hf[k_x][k_y][k_z][i]*f.volumeV()
  #define _hjfy(hf) v_y*hf[k_x][k_y][k_z][i]*f.volumeV()
  #define _velocity_vx(Ex,Ey,Bx,By) -( Ex[i] + v_y*B0 + v_z*By[i] )
  #define _velocity_vy(Ex,Ey,Bx,By) -( Ey[i] - v_x*B0 + v_z*Bx[i] )
  #define _velocity_vz(Ex,Ey,Bx,By) -( v_x*By[i] - v_y*Bx[i] )
  ...
  while ( current_t<c.Tf ) {
    ...
    // FIRST STAGE //////////////////////////////////////////////////
    {
      ...
      for ( auto k_x=0u ; k_x<c.Nvx ; ++k_x ) {
        const double v_x = k_x*f.step.dvx + f.range.vx_min;
        for ( auto k_y=0u ; k_y<c.Nvy ; ++k_y ) {
          const double v_y = k_y*f.step.dvy + f.range.vy_min;
          for ( auto k_z=0u ; k_z<c.Nvz ; ++k_z ) {
