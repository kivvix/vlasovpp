...
auto
perturbation ( double k , unsigned int Nmod ) {
  /*
  ublas::vector<double> kmods(Nmod);
  std::generate(
    kmods.begin() , kmods.end() ,
    [&,count=1] () mutable { return k*count++; }
  );
  return [=](double x){
    return std::accumulate(
        kmods.begin(),kmods.end(),
        0. ,
        [=](double r,double kn) { return r+std::cos(kn*x); }
      );
  };
  */
  return [=](double x){
    double r=0.;
    for (auto n=0;n<Nmod;++n) {
      r += std::cos(k*n*x);
    }
    return r;
  };
...
/* ------------------------------------------------------------------------- */
  ...
  auto pertu=perturbation(Kx,20);
  for (field<double,2>::size_type k=0 ; k<fh.size(0) ; ++k ) {
    for (field<double,2>::size_type i=0 ; i<fh.size(1) ; ++i ) {
      //fh[k][i] = ( tb_M1(Xi(i),Vk(k)) + tb_M2(Xi(i),Vk(k)) )*(1. + 0.1*std::cos(Kx*Xi(i)));
      fh[k][i] = ( tb_M1(Xi(i),Vk(k)) + tb_M2(Xi(i),Vk(k)) )*( 1. + 0.001*pertu(Xi(i)) );
    }
  ...
  std::vector<ublas::vector<double>> Et;
  ...
  auto save_data = [&] ( std::string && suffix ) -> void {
    ...
    // save Et (electric field at each time)
    std::ofstream of( c.output_dir / ("Et"+suffix+".dat") );
...
// MONITORING /////////////////////////////////////////////////////////////////
    ...
    iter.uc_error(uc,uctilde,fh.step.dx);
    ...
    ++iter.iter;
    //iter.dt = std::pow( c.tol/iter.error() , 0.25 )*iter.dt;
    //iter.dt = std::min(1.433*fh.step.dv/Emax.back(),2.);
