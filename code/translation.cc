...
int main(int,char**)
{
	std::size_t Nx = 81, Nv = 64 , Nb_iter=10;
	field<double,1> f(boost::extents[Nv][Nx]);

	f.range.v_min = -8.; f.range.v_max = 8.; f.step.dv = (f.range.v_max-f.range.v_min)/Nv;
	f.range.x_min = -8.; f.range.x_max = 8.; f.step.dx = (f.range.x_max-f.range.x_min)/Nx;

  // SIGMA is the CFL number
	const double dt = f.step.dv; //0.1;//SIGMA*f.step.dv/E_MAX;
	
	ublas::vector<double> v (Nv,0.);
  ublas::vector<double> E (Nx,1.);
  ...
  double Tf = 2.;//math::pi<double>()*0.5;
  int i_t=0;
...
#undef a52
    ...
    **/
    ...
    if ( i_t == int(0.1*Tf/dt) ) {
      std::stringstream ss; ss << "vp_" << SCHEME << "_01Tf.dat";
      f.write(ss.str());
      std::cout << std::endl;
    }
    if ( i_t == int(0.2*Tf/dt) ) {
      std::stringstream ss; ss << "vp_" << SCHEME << "_02Tf.dat";
      f.write(ss.str());
      std::cout << std::endl;
    }
    if ( i_t == int(0.3*Tf/dt) ) {
      std::stringstream ss; ss << "vp_" << SCHEME << "_03Tf.dat";
      f.write(ss.str());
      std::cout << std::endl;
    }
    if ( i_t == int(0.4*Tf/dt) ) {
      std::stringstream ss; ss << "vp_" << SCHEME << "_04Tf.dat";
      f.write(ss.str());
      std::cout << std::endl;
    }
    if ( i_t == int(0.5*Tf/dt) ) {
      std::stringstream ss; ss << "vp_" << SCHEME << "_05Tf.dat";
      f.write(ss.str());
      std::cout << std::endl;
    }
...
#undef L
  ...
  std::stringstream ss; ss << "vp_" << SCHEME << "_Tf.dat";
  f.write(ss.str());
  ss.str(std::string());
  std::ofstream of;
  std::size_t count = 0;
